#!/usr/bin/env python3
"""
Threat Validator AI Agent - COMPLETE mTLS Implementation

Combines:
1. All working logic from threat_validator_real_spiffe.py
2. Full mTLS server with client certificate verification
3. SPIFFE ID extraction and authorization
4. Integration with LM Studio Gateway for real AI validation
"""
...
logger = logging.getLogger('ThreatValidator-mTLS')
...
class ThreatValidatorHandler(BaseHTTPRequestHandler):
    """
    ...
    3. Processes threat validation requests
    4. Calls LM Studio Gateway for real AI validation
    ...
    def do_POST(self):
        ...
        # ================================================================
        # STEP 3: PROCESS REQUEST - Handle threat validation
        # ================================================================
        if self.path == '/validate':
            try:
                ...
                validate_data = json.loads(post_data.decode('utf-8'))
                
                logger.info(f"📥 Received validation request")
                logger.info(f"   From: {peer_spiffe_id}")
                logger.info(f"   Assessment: {validate_data.get('assessment', 'N/A')[:100]}")
                
                # Call LM Studio Gateway for real AI validation
                # Using mTLS with our SPIFFE certificate for mutual authentication
                try:
                    logger.info(f"🤖 Calling Gateway at {GATEWAY_URL}/validate (mTLS)")
                    ...
                    if gateway_response.status_code == 200:
                        ...
                        logger.info(f"   Validation: {result.get('validation', 'N/A')[:50]}...")
                        ...
                        response = {
                            ...
                            'validator_spiffe_id': self.mtls_handler.spiffe_id,
...
def main():
    """Main function to start the Threat Validator service with mTLS"""
    
    logger.info("=" * 70)
    logger.info("Threat Validator AI Agent - FULL mTLS Implementation")
    logger.info("=" * 70)
    ...
    ThreatValidatorHandler.mtls_handler = mtls_handler
    ...
    # Create HTTPS server
    port = 8445
    server = HTTPServer(('0.0.0.0', port), ThreatValidatorHandler)
    ...
    logger.info("=" * 70)
    logger.info("Ready to validate threats with mTLS authentication!")
