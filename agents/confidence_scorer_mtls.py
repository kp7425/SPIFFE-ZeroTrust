#!/usr/bin/env python3
"""
Confidence Scorer AI Agent - COMPLETE mTLS Implementation

Combines:
1. All working logic from confidence_scorer_real_spiffe.py
2. Full mTLS server with client certificate verification
3. SPIFFE ID extraction and authorization
4. Integration with LM Studio Gateway
"""
...
logger = logging.getLogger('ConfidenceScorer-mTLS')

# Gateway configuration
# NOTE: Gateway is HTTP because it's inside cluster and talks to localhost LM Studio
# If gateway also had mTLS, this would be https://
...
class SPIFFEMTLSHandler:
    ...
    def refresh_certificates(self):
        ...
        This runs at startup and could be called periodically for rotation
        """
        max_attempts = 5
...
class ConfidenceScorerHandler(BaseHTTPRequestHandler):
    """
    ...
    3. Processes confidence scoring requests
    4. Returns results with mTLS verification status
    """
    ...
    def do_POST(self):
        ...
        # ================================================================
        # STEP 3: PROCESS REQUEST - Handle confidence scoring
        # ================================================================
        if self.path == '/score':
            try:
                # Read request body
                ...
                logger.info(f"📥 Received confidence scoring request")
                logger.info(f"   From: {peer_spiffe_id}")
                logger.info(f"   Classification: {score_data.get('classification', 'N/A')[:100]}")
                
                # Call LM Studio Gateway for real AI confidence scoring
                # Using mTLS with our SPIFFE certificate for mutual authentication
                try:
                    logger.info(f"🤖 Calling Gateway at {GATEWAY_URL}/score (mTLS)")
                    ...
                    if gateway_response.status_code == 200:
                        ...
                        logger.info(f"   Raw response: {result.get('raw_response', 'N/A')[:50]}")
                        ...
                        response = {
                            ...
                            'scorer_spiffe_id': self.mtls_handler.spiffe_id,
...
def main():
    """Main function to start the Confidence Scorer service with mTLS"""
    
    logger.info("=" * 70)
    logger.info("Confidence Scorer AI Agent - FULL mTLS Implementation")
    logger.info("=" * 70)
    ...
    ConfidenceScorerHandler.mtls_handler = mtls_handler
    ...
    # Create HTTPS server
    port = 8444
    server = HTTPServer(('0.0.0.0', port), ConfidenceScorerHandler)
    ...
    logger.info("=" * 70)
    logger.info("Ready to score confidence with mTLS authentication!")
